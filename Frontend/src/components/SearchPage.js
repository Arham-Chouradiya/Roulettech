...
async function getIpInfo() {
    try {
        const response = await axios.get('https://ipinfo.io/json?token=e4bf73fa48f81d');
...
function SearchPage() {
    ...
    const get_suggestions_data = async (keyword) => {
        const suggestions_request = await axios.get(`http://localhost:8000/suggestions?keyword=${keyword}`);
    ...
    const get_table_data = async (event) => {
        ...
        const search_result = await axios.get(`http://localhost:8000/search_events?keyword=${keyword}&distance=${distance}&category=${category}&latitude=${latitude}&longitude=${longitude}`);
    ...
    const get_event_data = async (event_id, venue_name, event_content) => {
        const event_result = await axios.get(`http://localhost:8000/event_details?event_id=${event_id}`);
        set_event_content(event_result.data);
        const venue_result = await axios.get(`http://localhost:8000/venue_details?keyword=${venue_name}`);
        ...
        for(var i = 0; i < event_content._embedded.attractions.length; i++) {
                const spotify_result = await axios.get(`http://localhost:8000/spotify_details?keyword=${event_content._embedded.attractions[i].name}`);
                spotify_data[i] = spotify_result.data;
                if(i===0) {set_first_spotify(spotify_data[i].name)};
                const spotify_album_data = await axios.get(`http://localhost:8000/spotify_album_details?keyword=${spotify_result.data.id}`);
